...
/*
  ...
  result = readPsu(PSU_READ_ADDR, 12, PSU_SERIAL_NUMBER);
  Serial.print("Serial Number: ");
  for (uint8_t i = 0; i < 12; i++)
  {
    Serial.print(char(in_buffer[i]));
  }
  Serial.println();
  ...
  result = readPsu(PSU_READ_ADDR, 8, PSU_MODEL_NUMBER);
  ...
  Serial.print("Temp Sec Diodes: ");
  Serial.println(in_buffer[0]);

  result = readPsu(PSU_READ_ADDR, 2, PSU_HVSD_DEFAULT);
  Serial.print("HVSD: ");
  Serial.println(in_buffer[0] + (in_buffer[1] << 8));  

  result = readPsu(PSU_READ_ADDR, 2, PSU_VSET_DEFAULT);
  Serial.print("VSET: ");
  Serial.println(in_buffer[0] + (in_buffer[1] << 8));  

  result = readPsu(PSU_READ_ADDR, 2, PSU_ISET_DEFAULT);
  Serial.print("ISET: ");
  Serial.println(in_buffer[0] + (in_buffer[1] << 8));  
  ...
  write16Psu(PSU_WRITE_ADDR, PSU_VSET_DEFAULT, 5000);
  Serial.println("Write VSET_DEFAULT");
  delay(1000);
